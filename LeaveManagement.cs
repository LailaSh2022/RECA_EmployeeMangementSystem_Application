    ...
    public partial class LeaveManagement : Form
    {
        ...
        private void fillLables()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                remSickLeaveFill_lbl.Text = dr["Annual_Leave"].ToString(); 
            }
            if (counter == 0)
            {
            ...
            try
            {
                using (MySqlConnection connection = new MySqlConnection(ConnectionString))
                {
                    using (MySqlCommand command = new MySqlCommand())
                    {
                        ...
                        command.CommandText = "INSERT INTO human_resources.leave (LE_ID, Emp_ID, LE_Type, LE_From_Date, LE_To_Date, LE_Balance, LE_Reason, LE_Docs) " +
                            "VALUES (?LE_ID, ?Emp_ID, ?LE_Type, ?LE_From_Date, ?LE_To_Date, ?LE_Balance, ?LE_Reason, ?LE_Docs)";
