        ...
        /*private void btnLogin_Click(object sender, EventArgs e)
        {
            try
            {
                if (textBox1.Text != "" && textBox2.Text != "")
                {
                    ...
                    con.Open();
                    string query = "select Emp_ID,Emp_User_Name,Emp_password,Emp_Fname from human_resources.employee WHERE Emp_User_Name ='" + textBox1.Text + "' AND Emp_password ='" + textBox2.Text + "'";   // changing username to Emp_User_Name, password to Emp_password and human.resources.employee to human_resources.employee
                    MySqlDataReader row;
                    row = con.ExecuteReader(query);
                    if (row.HasRows)
                    {
                        ...
                        {
                            id = row["Emp_ID"].ToString();
                            username = row["Emp_User_Name"].ToString();
                            password = row["Emp_password"].ToString();
                            firstname = row["Emp_FName"].ToString();
